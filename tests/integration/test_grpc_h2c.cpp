...
int main() {
    ...
    if (!proxyPortOpt) return 1;
    const uint16_t proxyPort = *proxyPortOpt;

    EventLoop loop;
    proxy::ProxyServer server(&loop, proxyPort, "roundrobin", "GrpcH2cProxy");
    server.Start();
    ...
    std::thread client([&]() {
        ...
        // Read server SETTINGS and ACK, then ACK server SETTINGS.
        ...
        // Build unary request body (grpc frame containing protobuf(field1=msg)).
        const std::string msg = "hi";
        std::string pb;
        proxy::protocol::ProtobufLite::EncodeStringField1(msg, &pb);
        std::string frame;
        proxy::protocol::GrpcFramer::EncodeMessage(pb, &frame);

        // Send unary and server-streaming requests back-to-back (multiplex).
        {
            std::vector<proxy::protocol::Hpack::Header> h = {
                {":method", "POST"},
                {":path", "/proxy.Echo/Unary"},
                {":scheme", "http"},
                {":authority", "127.0.0.1"},
                {"content-type", "application/grpc"},
                {"te", "trailers"},
            };
            auto fr = makeHeaders(1, h, false);
        ...
        }
        {
            std::vector<proxy::protocol::Hpack::Header> h = {
                {":method", "POST"},
                {":path", "/proxy.Echo/Stream"},
                {":scheme", "http"},
                {":authority", "127.0.0.1"},
                {"content-type", "application/grpc"},
                {"te", "trailers"},
            };
            auto fr = makeHeaders(3, h, false);
            sendAll(fd, fr.data(), fr.size());
            auto d = makeData(3, frame, true);
            sendAll(fd, d.data(), d.size());
        }
        ...
        std::unordered_map<uint32_t, StreamAcc> acc;
        acc[1] = StreamAcc{};
        acc[3] = StreamAcc{};

        proxy::protocol::Hpack dec;
        while (!(acc[1].gotTrailers && acc[3].gotTrailers)) {
            Frame f;
            assert(recvFrame(fd, &buf, &f, 3000));
            if (f.h.streamId == 0) continue;
            auto it = acc.find(f.h.streamId);
            ...
            if (f.h.type == static_cast<uint8_t>(proxy::protocol::Http2FrameType::kHeaders)) {
                ...
                for (const auto& h : hs) {
                    if (h.name == ":status") {
                        ...
                        it->second.grpcStatus = h.value;
                    } else if (h.name == "content-type") {
                        assert(h.value.find("application/grpc") != std::string::npos);
                    }
                }
                if (hasStatus) it->second.gotInitialHeaders = true;
                if (f.h.flags & 0x1) it->second.gotTrailers = true;
                continue;
            }

            if (f.h.type == static_cast<uint8_t>(proxy::protocol::Http2FrameType::kData)) {
                it->second.data.append(reinterpret_cast<const char*>(f.p.data()), f.p.size());
                continue;
            }
        }

        assert(acc[1].gotInitialHeaders);
        assert(acc[3].gotInitialHeaders);
        assert(acc[1].grpcStatus == "0");
        assert(acc[3].grpcStatus == "0");

        {
            std::vector<std::string> msgs;
            assert(proxy::protocol::GrpcFramer::DecodeMessages(reinterpret_cast<const uint8_t*>(acc[1].data.data()), acc[1].data.size(), &msgs));
            assert(msgs.size() == 1);
            std::string outText;
        ...
        {
            std::vector<std::string> msgs;
            assert(proxy::protocol::GrpcFramer::DecodeMessages(reinterpret_cast<const uint8_t*>(acc[3].data.data()), acc[3].data.size(), &msgs));
            assert(msgs.size() == 3);
            std::string out1, out2, out3;
            assert(proxy::protocol::ProtobufLite::DecodeStringField1(reinterpret_cast<const uint8_t*>(msgs[0].data()), msgs[0].size(), &out1));
            assert(proxy::protocol::ProtobufLite::DecodeStringField1(reinterpret_cast<const uint8_t*>(msgs[1].data()), msgs[1].size(), &out2));
            assert(proxy::protocol::ProtobufLite::DecodeStringField1(reinterpret_cast<const uint8_t*>(msgs[2].data()), msgs[2].size(), &out3));
            assert(out1 == "hi#1");
            assert(out2 == "hi#2");
            assert(out3 == "hi#3");
        }
