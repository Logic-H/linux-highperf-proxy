...
int main() {
    ...
    EventLoop loop;
    proxy::ProxyServer server(&loop, proxyPort, "roundrobin", "HistProxy");
    ...
    std::thread client([&]() {
        ...
        // Generate some traffic to produce non-zero qps.
        for (int i = 0; i < 5; ++i) {
            int fd = connectTo(proxyPort);
            assert(fd >= 0);
            const std::string req = "GET /hello HTTP/1.1\r\nHost: test\r\nAccept-Encoding: identity\r\nConnection: close\r\n\r\n";
            sendAll(fd, req);
            const std::string resp = readAllWithTimeout(fd, 1000);
            ::close(fd);
            assert(resp.find("200 OK") != std::string::npos);
        }

        // Wait for at least a few samples.
        std::this_thread::sleep_for(std::chrono::milliseconds(600));

        int fd = connectTo(proxyPort);
        assert(fd >= 0);
        const std::string req = "GET /history?seconds=2 HTTP/1.1\r\nHost: test\r\nConnection: close\r\n\r\n";
        sendAll(fd, req);
        const std::string resp = readAllWithTimeout(fd, 1500);
        ::close(fd);

        assert(resp.find("200 OK") != std::string::npos);
        assert(resp.find("\"points\"") != std::string::npos);
        assert(resp.find("\"qps\"") != std::string::npos);

        int fd2 = connectTo(proxyPort);
        assert(fd2 >= 0);
        const std::string req2 = "GET /history/summary?seconds=2 HTTP/1.1\r\nHost: test\r\nConnection: close\r\n\r\n";
        sendAll(fd2, req2);
        const std::string resp2 = readAllWithTimeout(fd2, 1500);
        ::close(fd2);
        assert(resp2.find("200 OK") != std::string::npos);
        assert(resp2.find("\"qps\"") != std::string::npos);
